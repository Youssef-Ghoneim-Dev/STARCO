...
function RegisterForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!/^\+?[1-9]\d{7,14}$/.test(formData.phoneNumber.replace(/\s|-/g, ""))) return setPhoneError("Enter a valid phone number with the country code.");
    setLoading(true);
    try {
      await register({ name: formData.name, email: formData.email, phoneNumber: formData.phoneNumber, password: formData.password, role: formData.role });
  ...
  return <div className="auth-card auth-register-card">
    ...
    <form onSubmit={handleSubmit} className="auth-body" style={{ width: "100%" }}>
      ...
      <div className="phone-input-field"><AuthInput label="Phone number" type="tel" name="phoneNumber" placeholder="2010xxxxxxxx" value={formData.phoneNumber} onChange={handleChange} />{phoneError && <p className="field-error">{phoneError}</p>}</div>
    ...
    {import.meta.env.VITE_GOOGLE_CLIENT_ID && <div className="google-auth"><span>أو</span><div className="google-login-shell"><span><FaGoogle />Sign up with Google</span><GoogleLogin onSuccess={signUpWithGoogle} onError={() => toast.error("تعذر الاتصال بـ Google.")} text="signup_with" theme="outline" shape="pill" size="large" width="300" /></div></div>}

...
import { currentAction, deliveryDate, isThisMonth, itemClient, itemCode, itemDate, itemLink, itemName, manufacturingFilesUploadedOn, sameDay, statusLabel, statusMeta, statusProgress, taskOutcome, workflowAverages } from "../../utils/dashboardData";
...
function EngineerDashboard({ name, userId, panels = [], loading, onRefresh }) {
  ...
  return <div className="engineer-dashboard" dir="rtl">
    ...
    <section className="engineer-main-grid">
      <section className="engineer-panel engineer-quick-actions"><h2>إجراءات سريعة</h2><Link className="primary" to="/panels?view=engineerTasks"><HiOutlineClock />المهام المنتظرة</Link><Link to="/panels?view=executionPdfs&statuses=executionPdfRequested,executionPdfReady"><HiOutlineCloudUpload />لوحات PDF التنفيذ</Link><Link to="/panels?view=manufacturingFiles&statuses=manufacturingFilesPending,manufacturingFilesReady"><HiOutlineFolder />ملفات التصنيع</Link><Link to="/projects"><HiOutlineFolder />مشاريعي</Link><Link to="/configuration"><HiOutlineTemplate />الإعدادات والقوالب</Link></section>
      <TaskList title={`طلبات ${selectedLabel}`} items={tasks} allLink={`/panels?view=requests&date=${selectedValue}`} />
      <section className="engineer-panel engineer-execution-card"><h2>المهام التي تنتظر إجراءك</h2><div>{pendingTasks.map((panel) => <Link to={itemLink(panel)} key={panel._id}><div><strong><DashboardName>{itemName(panel)}</DashboardName></strong><span>{itemCode(panel)} · {currentAction(panel, "Engineer")}</span></div><small>قيد الانتظار</small></Link>)}{!pendingTasks.length && <p className="engineer-empty">أنهيت كل المهام الحالية</p>}</div><Link className="engineer-more-link" to="/panels?view=engineerTasks">عرض كل المهام <HiOutlineExternalLink /></Link></section>

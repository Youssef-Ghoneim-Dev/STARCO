...
function EngineerDashboard({ name, userId, panels = [], loading, onRefresh, title = "Drawing Engineer" }) {
  ...
  return <><div className="engineer-dashboard" dir="rtl">
    <header className="engineer-dashboard-header"><div><h1>لوحة التحكم - {title}</h1><p>مرحبًا {name || "بك"}، جميع الأرقام محسوبة من سجل اللوحات الفعلي.</p></div><div className="engineer-date-tools"><button type="button" onClick={onRefresh} disabled={loading}><HiOutlineRefresh className={loading ? "dashboard-refresh-spinning" : ""} />{loading ? "جاري التحديث..." : "تحديث"}</button><label className="engineer-date-input" onClick={openPicker}><HiOutlineCalendar /><input ref={dateRef} type="date" value={selectedValue} min={dateValue(minimumDate)} max={dateValue(today)} onChange={(event) => setSelectedValue(event.target.value)} /></label><div className="engineer-period-select"><StyledSelect value={preset} onChange={(value) => setSelectedValue(value === "today" ? dateValue(today) : value === "yesterday" ? dateValue(yesterday) : selectedValue)} options={[{ value: "today", label: "اليوم" }, { value: "yesterday", label: "أمس" }, ...(preset === "custom" ? [{ value: "custom", label: "تاريخ محدد" }] : [])]} /></div></div></header>

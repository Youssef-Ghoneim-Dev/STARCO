...
function MarketingManagerDashboard({ name, projects, panels = [], users = [], loading, onRefresh }) {
  ...
  const productionStages = [["تجميع", panels.filter((panel) => panel.status === "assembly").length], ["رش", panels.filter((panel) => panel.status === "painting").length], ["تصنيع", panels.filter((panel) => panel.status === "manufacturing").length], ["ليزر", panels.filter((panel) => panel.status === "laser").length]];
  ...
  return <div className="marketing-dashboard" dir="rtl">
    ...
    <section className="marketing-metrics-grid"><MarketingMetric tone="green" icon={<HiOutlineCheckCircle />} title="مشاريع مكتملة" value={statusCounts.completed} note="حسب الحالة الفعلية" /><MarketingMetric tone="purple" icon={<HiOutlineChartBar />} title="مشاريع تحت التنفيذ" value={statusCounts.production} note="من مراحل اللوحات" /><MarketingMetric tone="amber" icon={<HiOutlineClock />} title="أوامر تنفيذ قيد الانتظار" value={statusCounts.execution} note="تحتاج متابعة" /><MarketingMetric tone="cyan" icon={<HiOutlineShoppingBag />} title={`طلبات العملاء (${label})`} value={currentProjects.length} note={`${currentProjects.length - previousProjects.length >= 0 ? "+" : ""}${currentProjects.length - previousProjects.length} مقارنة باليوم السابق`} /><MarketingMetric tone="emerald" icon={<HiOutlineBriefcase />} title={`المشاريع الجديدة (${label})`} value={newToday} note="مشاريع مسجلة" /><MarketingMetric tone="blue" icon={<HiOutlineUsers />} title="إجمالي المسوقين" value={marketers.length} note="من قائمة المستخدمين" /></section>

    <section className="marketing-layout">
      <section className="marketing-panel marketing-delay"><h2>أسباب التأخير المسجلة</h2>{delayReasons.slice(0, 5).map(([reason, count]) => <div key={reason}><b>{count}</b><span><strong>{reason}</strong><small>{count} تسجيل</small></span></div>)}{!delayReasons.length && <p className="marketing-empty">لا توجد أسباب تأخير مسجلة</p>}<Link to="/panels?view=delayed&delayed=true" className="marketing-card-link">عرض اللوحات <HiOutlineExternalLink /></Link></section>
      ...
      <section className="marketing-panel marketing-production"><h2>اللوحات في مراحل الإنتاج</h2><div>{productionStages.map(([stage, count]) => <article key={stage}><span>{stage}</span><strong>{count}</strong><small>لوحة</small></article>)}</div>{delayedPanels.length ? <p><HiOutlineExclamation />لوحات متأخرة وفق البيانات المسجلة <strong>{delayedPanels.length}</strong></p> : <p><HiOutlineCheckCircle />لا توجد لوحات متأخرة حاليًا</p>}</section>
      <section className="marketing-panel marketing-team"><h2>أداء المسوقين ({label})</h2><div className="marketing-table-scroll"><table><thead><tr><th>المسوق</th><th>مشاريع جديدة</th><th>إجمالي المشاريع</th><th>مكتملة</th><th>معدل الإكمال</th></tr></thead><tbody>{marketerRows.map((row) => <tr key={row[0]}>{row.map((cell, index) => <td key={index}>{cell}</td>)}</tr>)}</tbody></table></div><Link className="marketing-card-link" to="/users">عرض فريق التسويق <HiOutlineExternalLink /></Link></section>

      <section className="marketing-panel marketing-alerts"><h2>تنبيهات مهمة</h2>{delayedPanels.length ? <p><HiOutlineExclamation />{delayedPanels.length} لوحات متأخرة وتحتاج متابعة</p> : <p><HiOutlineCheckCircle />لا توجد تنبيهات تأخير حقيقية الآن</p>}<Link className="marketing-card-link" to="/panels?view=delayed&delayed=true"><HiOutlineBell />عرض اللوحات</Link></section>
      ...
      <section className="marketing-panel marketing-best"><h2>أعلى المسوقين إكمالًا</h2>{[...marketerRows].sort((a, b) => b[3] - a[3]).slice(0, 4).map((row, index) => <div key={row[0]}><b>{index + 1}</b><span>{row[0]}</span><small>{row[3]} مكتملة</small></div>)}<Link className="marketing-card-link" to="/users">عرض الفريق <HiOutlineExternalLink /></Link></section>

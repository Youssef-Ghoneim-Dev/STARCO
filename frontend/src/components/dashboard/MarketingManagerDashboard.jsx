...
function MarketingManagerDashboard({ name, projects, panels = [], users = [], loading, onRefresh }) {
  ...
  return <div className="marketing-dashboard" dir="rtl">
    ...
    <section className="marketing-layout">
      <section className="marketing-panel marketing-delay"><h2>أسباب التأخير المسجلة</h2>{delayReasons.slice(0, 5).map(([reason, count]) => <div key={reason}><b>{count}</b><span><strong>{reason}</strong><small>{count} تسجيل</small></span></div>)}{!delayReasons.length && <p className="marketing-empty">لا توجد أسباب تأخير مسجلة</p>}<Link to="/panels" className="marketing-card-link">عرض اللوحات <HiOutlineExternalLink /></Link></section>

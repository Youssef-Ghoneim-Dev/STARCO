...
function ProductionManagerDashboard({ name, panels = [], loading, onRefresh }) {
  ...
  const pdfReady = panels.filter((project) => ["executionPdfRequested", "executionPdfReady"].includes(project.status)).length;
  ...
  const newOrders = [...panels].sort((a, b) => updatedAt(b) - updatedAt(a)).filter((project) => ["executionPdfRequested", "executionPdfReady", "executionConfirmed", "manufacturingFilesPending"].includes(project.status) && sameDay(project.executionPdf?.requestedAt || project.updatedAt || project.createdAt, selectedDate)).slice(0, 5);

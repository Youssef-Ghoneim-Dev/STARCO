...
function StageDonut({ counts, total, onViewAll }) {
  const segments = stages.map((stage) => ({ ...stage, value: counts[stage.key] || 0 }));
  return <section className="production-manager-card production-stage-status"><h2>اللوحات حسب المرحلة</h2><div className="production-stage-status-body"><DashboardDonut className="production-stage-donut" segments={segments} total={total} /><div>{stages.map((stage) => <p key={stage.key}><i style={{ background: stage.color }} /><span>{stage.label}</span><strong>{counts[stage.key]}</strong><small>{total ? `${Math.round((counts[stage.key] / total) * 100)}%` : "0%"}</small></p>)}</div></div><button type="button" onClick={onViewAll} className="production-card-link">عرض اللوحات حسب المرحلة <HiOutlineExternalLink /></button></section>;
}

function ProductionManagerDashboard({ name, panels = [], loading, onRefresh, role = "ProductionManager", title = "Production Manager" }) {
  ...
  const allWaiting = sorted.filter((project) => currentAction(project, role));
  ...
  const newOrders = [...productionItems]
    .filter((panel) => panel.status === "manufacturingFilesPending" && sameDay(panel.executionPdf?.confirmedAt, selectedDate))
  ...
  const reasonSegments = delayReasons.map(([reason, count], index) => ({ key: `${reason}-${index}`, label: reason, value: count, color: reasonColors[index % reasonColors.length] }));
  ...
  return <><div className="production-manager-dashboard" dir="rtl">
    ...
    <section className="production-manager-grid">
      <section className="production-manager-card production-delay-reasons"><h2>أسباب التأخير المسجلة</h2><div><DashboardDonut className="production-reasons-donut" segments={reasonSegments} total={totalReasons} totalLabel="إجمالي أسباب التأخير" /><section>{delayReasons.map(([reason, count], index) => <p className="dashboard-status-row" data-tooltip={`${reason}: ${count}`} key={reason}><i style={{ background: reasonColors[index % reasonColors.length] }} /><span>{reason}</span><strong>{count}</strong><small>{totalReasons ? `${Math.round((count / totalReasons) * 100)}%` : "0%"}</small></p>)}{!delayReasons.length && <div className="production-empty production-empty-reasons">لا توجد أسباب تأخير مسجلة</div>}</section></div><button type="button" onClick={() => setDetailsModal("delayed")} className="production-card-link">عرض اللوحات المتأخرة <HiOutlineExternalLink /></button></section>
      <StageDonut counts={stageCounts} total={productionItems.length} onViewAll={() => setDetailsModal("stages")} />
      <section className="production-manager-card production-delayed-list"><h2>اللوحات المتأخرة حسب المرحلة</h2><div>{delayed.slice(0, 4).map((project, index) => <Link to={itemLink(project)} key={project._id || index}><span><strong><DashboardName>{projectName(project)}</DashboardName></strong><small>{projectCode(project)} · {statusLabel(project)}</small></span><b>متأخر {daysLate(project, selectedDateEnd)} يوم</b></Link>)}{!delayed.length && <p className="production-empty">لا توجد لوحات متأخرة</p>}</div><button type="button" onClick={() => setDetailsModal("delayed")} className="production-card-link">عرض جميع اللوحات المتأخرة <HiOutlineExternalLink /></button></section>
      <section className="production-manager-card production-stage-progress"><h2>تقدم مراحل الإنتاج</h2>{stages.slice(1, 5).map((stage) => { const percent = productionItems.length ? Math.round((stageCounts[stage.key] / productionItems.length) * 100) : 0; return <div key={stage.key}><span>{stage.label.replace("في ", "")}</span><strong>{stageCounts[stage.key]}</strong><i><b style={{ width: `${percent}%`, background: stage.color }} /></i><em>{percent}%</em></div>; })}<button type="button" onClick={() => setDetailsModal("progress")} className="production-card-link">عرض التفاصيل <HiOutlineExternalLink /></button></section>
  ...
  </div>{showAllTasks && <DashboardTasksModal title={`كل مهام ${title}`} subtitle="اللوحات التي تحتاج اعتماد موعد أو تحديد موعد بديل أو تحديثًا في مرحلة الإنتاج." items={allWaiting.map((panel) => ({ panel, action: currentAction(panel, role) }))} onClose={() => setShowAllTasks(false)} />}{detailsModal && <DashboardTasksModal eyebrow="تفاصيل اللوحات" title={detailsTitle} subtitle={detailsModal === "delayed" ? "لوحات غير مكتملة تجاوزت موعد التسليم المعتمد." : "اللوحات موزعة وفق مرحلة الإنتاج الحالية المسجلة."} items={detailsItems} emptyMessage={detailsModal === "delayed" ? "لا توجد لوحات متأخرة حاليًا." : "لا توجد لوحات في دورة الإنتاج حاليًا."} onClose={() => setDetailsModal(null)} />}</>;
}

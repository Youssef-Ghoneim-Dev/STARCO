...
function ProductionManagerDashboard({ name, panels = [], loading, onRefresh, role = "ProductionManager", title = "Production Manager" }) {
  ...
  return <><div className="production-manager-dashboard" dir="rtl">
    ...
    <section className="production-manager-grid">
      ...
      <section className="production-manager-card production-waiting"><h2>اللوحات التي تنتظر إجراءك</h2>{waiting.map((project, index) => <Link to={itemLink(project)} key={project._id || index}><i><HiOutlineClock /></i><span><strong><DashboardName>{projectName(project)}</DashboardName></strong><small>{currentAction(project, role)}</small></span><b>عرض</b></Link>)}{!waiting.length && <p className="production-empty">لا توجد لوحات تنتظر إجراءً</p>}<button type="button" onClick={() => setShowAllTasks(true)} className="production-card-link">عرض كل المهام <HiOutlineExternalLink /></button></section>

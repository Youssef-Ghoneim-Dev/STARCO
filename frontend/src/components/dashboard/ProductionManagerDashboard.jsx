...
function ProductionManagerDashboard({ name, panels = [], loading, onRefresh }) {
  ...
  return <div className="production-manager-dashboard" dir="rtl">
    ...
    <section className="production-manager-grid">
      ...
      <section className="production-manager-card production-delayed-list"><h2>اللوحات المتأخرة حسب المرحلة</h2><div>{(delayed.length ? delayed : sorted).slice(0, 4).map((project, index) => <Link to={itemLink(project)} key={project._id || index}><span><strong>{projectName(project)}</strong><small>{projectCode(project)} · {stages.find((stage) => stage.key === resolveStage(project))?.label}</small></span><b>متأخر {index + 1} يوم</b></Link>)}</div><Link to="/panels" className="production-card-link">عرض جميع اللوحات المتأخرة <HiOutlineExternalLink /></Link></section>
      ...
      <section className="production-manager-card production-waiting"><h2>اللوحات التي تنتظر إجراءك</h2>{waiting.map((project, index) => <Link to={itemLink(project)} key={project._id || index}><i><HiOutlineClock /></i><span><strong>{projectName(project)}</strong><small>{index % 2 ? "بانتظار بدء المرحلة التالية" : "لم يتم إسنادها للمرحلة"}</small></span><b>عرض</b></Link>)}{!waiting.length && <p className="production-empty">لا توجد لوحات تنتظر إجراءً</p>}</section>

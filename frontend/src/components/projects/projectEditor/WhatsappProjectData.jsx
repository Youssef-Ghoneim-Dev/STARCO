...
import { getProjectMedia, getProjectMediaFile } from "../../../services/projectsAPI";
...
function WhatsappProjectData() {
  ...
  return <section className="whatsapp-project-data" dir="rtl">
    <div className="whatsapp-data-heading"><h2>بيانات المشروع</h2><p>هذه البيانات أرسلها المندوب عبر WhatsApp، وهي للعرض فقط.</p></div>
    <div className="whatsapp-readonly-grid"><ReadOnlyField label="نوع اللوحة" value={panel.panelType} /><ReadOnlyField label="هل يوجد نحاس" value={panel.hasCopper === true ? "نعم" : panel.hasCopper === false ? "لا" : ""} />{panel.panelType === "كنترول" && <ReadOnlyField label="تركيب لوحة الكنترول" value={panel.controlInstallation} />}<ReadOnlyField label="تفاصيل إضافية" value={panel.additionalDetails} /></div>

...
export default function PanelCard({ panel, quotePublished = false, onOpen, onDelete, canDelete, onEdit }) {
  ...
  return <article ref={menuRef} className="panel-folder-card" onClick={onOpen} dir="rtl">{unreadNotification && <span className="panel-unread-indicator" role="status" aria-label="توجد إشعارات جديدة لهذه اللوحة" title={unreadNotification.title || "إشعار جديد"} />}<div className="panel-folder-card-top"><span className={`panel-folder-status ${tone}`}>{label}</span><button type="button" onClick={(event) => { event.stopPropagation(); setMenu((value) => !value); }}><HiOutlineDotsVertical /></button></div>
    ...
    <div className="panel-folder-icon">P{String(panel.sequence).padStart(2, "0")}</div><h2><bdi dir={getPanelNameDirection(panel.panelName)}>{panel.panelName || `لوحة ${panel.sequence}`}</bdi></h2><code>{panel.panelCode}</code><dl><div><dt>النوع</dt><dd>{panel.panelType || panel.marketerData?.panelType || "غير محدد"}</dd></div>{!isProductionSupervisor && <div><dt>المهندس</dt><dd>{user?.role === "OwnerManager" && panel.assignedEngineer?.name ? panel.assignedEngineer.name : panel.engineerId ? "تم الإسناد" : "غير مسندة"}</dd></div>}</dl></article>;
}

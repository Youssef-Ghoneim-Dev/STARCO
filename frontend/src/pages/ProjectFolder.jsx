...
export default function ProjectFolder() {
  ...
  const generatePreview = async () => { setBusy(true); try { const { data } = await completeProject(id); await navigator.clipboard.writeText(data.previewUrl); if (data.notificationMessage) toast.error(data.notificationMessage, { duration: 7000 }); await load(); } catch (error) { toast.error(error.response?.data?.message || "تعذر حفظ المشروع وإرساله للمندوب."); } finally { setBusy(false); } };

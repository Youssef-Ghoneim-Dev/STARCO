...
import { acquireProjectSetupLock, completeProject, completeProjectSetup, createPanel, deletePanelRecord, getProject, submitMarketingProject, submitPanelEdits } from "../services/projectsAPI";
...
export default function ProjectFolder() {
  ...
  const submitEdits = async () => { setSubmittingProject(true); try { await submitPanelEdits(id); toast.success("تم إرسال التعديلات للمهندسين بنجاح."); await load(); } catch (error) { toast.error(error.response?.data?.message || "تعذر إرسال التعديلات."); } finally { setSubmittingProject(false); } };
